...
<body>
    ...
    <div id="mainmenu">
        <div id="sidemenu">
            <h1>Summary</h1>
            <?php
            $sqlcon = "SELECT count(*) as total from ratings where user_id = '2'";
            $resultcon = mysqli_query($conn, $sqlcon);
            $data = mysqli_fetch_assoc($resultcon);
            $numberofratings = $data['total'];


            $sqlsum = "SELECT SUM(rating) as sum_rating from ratings where user_id = '2'";
        ...
        <table class="content-table">
            ...
            <tbody>
                <?php
                $sql = "SELECT * from ratings WHERE user_id = '2'";
